...
const SuggestionPanel: React.FC<SuggestionPanelProps> = ({ suggestions, isLoading, error, isPlaying, onSelect }) => {
  return (
    <div className="flex flex-col h-full bg-slate-900/40 rounded-[2rem] border border-slate-700/50 p-6 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-black text-slate-300 uppercase tracking-wider">How to Respond</h3>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-hide">
        {error ? (
          <div className="p-4 text-center border border-red-500/20 rounded-2xl bg-red-900/10 text-[10px] text-red-400 font-medium leading-relaxed">
            <p className="mb-1 uppercase font-bold tracking-wider">Quota Notice</p>
            {error}
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-48 text-slate-600">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-40">Awaiting Turn</p>
          </div>
        ) : (
          suggestions.map((text, idx) => (
            ...
            >
              <div className="pr-10">
                <span className="text-[9px] font-bold text-blue-500/60 uppercase tracking-widest mb-1 block">Option {idx + 1}</span>
      ...
      <div className="mt-6 flex flex-col gap-2">
         ...
         <div className="flex items-center gap-2">
           ...
           <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Learning Tip</p>
         </div>
         <p className="text-[10px] text-slate-400 leading-normal italic">
           Repeat the selected phrase three times after listening to internalize the intonation.
         </p>
      </div>
    </div>
  );
